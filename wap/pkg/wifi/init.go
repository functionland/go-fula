...
func execWithJournalctlCallback(ctx context.Context, command string, conclusionMessage *string) error {
	err := j.SeekTail()
	if err != nil {
		return fmt.Errorf("Error seeking to the end of the journal: %s", err)
	}
	...
	for {
		...
		for entryExist > 0 {
			...
			if err != nil {
				return fmt.Errorf("Error reading the message field: %s", err)
			}
			cmdLine, err := j.GetData(sdjournal.SD_JOURNAL_FIELD_CMDLINE)
			if err != nil {
				log.Errorf("Error reading the cmdline field:: %s", err)
				continue
			}
			if msg == "pam_unix(sudo:session): session closed for user root" && cmdLine == command {
				return nil
			}
			entryExist, err = j.Next()
		}
		select {
		case <-ctx.Done():
			return fmt.Errorf("context error: %v", ctx.Err())
