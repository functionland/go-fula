...
import (
	...
	"github.com/functionland/go-unixfs/importer/balanced"
	ihelper "github.com/functionland/go-unixfs/importer/helpers"
	"github.com/functionland/go-unixfs/importer/trickle"
...
type EncodedFileImpl struct {
	files.File
	jwe []byte
}

type EncodedFile interface {
	files.File
	JWE() []byte
}

func NewEncodedFileFromNode(n files.File, jwe []byte) *EncodedFileImpl {
	return &EncodedFileImpl{
		n,
		jwe,
	}
}

func (ef *EncodedFileImpl) JWE() []byte {
	return ef.jwe
}
...
func (adder *Adder) addWJwe(reader io.Reader, jwe []byte) (ipld.Node, error) {
	chnk, err := chunker.FromString(reader, adder.Chunker)
	if err != nil {
		return nil, err
	}
	...
	if adder.Trickle {
		...
		nd, err = balanced.LayoutEnc(db, jwe)
	}
	if err != nil {
		return nil, err
	}
...
func (adder *Adder) addFileNode(ctx context.Context, path string, file files.Node, toplevel bool) error {
	...
	case EncodedFile:
		return adder.addEncodedFile(path, f)
	case files.File:
...
func (adder *Adder) addEncodedFile(path string, file EncodedFile) error {
	var f files.File = file.(files.File)
	...
	dagnode, err := adder.addWJwe(reader, file.JWE())
	if err != nil {
		return err
	}
